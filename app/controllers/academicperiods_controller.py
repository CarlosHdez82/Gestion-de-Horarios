...
class PeriodsController:
    ...
    def get_periods(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            # Seleccionamos según las nuevas columnas
            cursor.execute("SELECT id, name, is_active, created_at, updated_at FROM academic_periods ORDER BY id DESC")
    ...
    def get_period(self, id: int):
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT id, name, is_active, created_at, updated_at FROM academic_periods WHERE id = %s", (id,))
    ...
    def create_period(self, data: AcademicPeriodCreate):
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            # SQL ajustado a la nueva tabla
            cursor.execute("""
                INSERT INTO academic_periods (name, is_active)
    ...
    def delete_period(self, id: int):
        ...
        try:
            ...
            if conn: conn.rollback()
            # Error de integridad si hay registros en teacher_availability o schedules
